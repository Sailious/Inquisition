...
日志截图等图片可写入七牛云 Kodo（私有空间）。上传后返回的**签名下载 URL** 带有效期，默认 30 天（与 COS 分支对齐）。过期后旧链接将无法访问，可按需通过环境变量调整。

| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `QINIU_URL_EXPIRE_SECONDS` | `2592000` | 七牛签名下载 URL 有效期（秒），默认 2592000（30 天）；配置为 0 或负数时回退默认 30 天 |
